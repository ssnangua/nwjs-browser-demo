  ...
  class ChromeTabs {
    ...
    setupDraggabilly() {
      ...
      tabEls.forEach((tabEl, originalIndex) => {
        ...
        draggabilly.on('pointerDown', (_, event) => {
          this.setCurrentTab(tabEl)
          // HACK
          this.emit('tabMousedown', { event, tabEl })
        })

        draggabilly.on('dragStart', (_, event) => {
          this.isDragging = true
          this.draggabillyDragging = draggabilly
          tabEl.classList.add('chrome-tab-is-dragging')
          this.el.classList.add('chrome-tabs-is-sorting')
          // HACK
          this.emit('dragStart', { event, tabEl })
        })

        draggabilly.on('dragEnd', (_, event) => {
          this.isDragging = false
        ...
        draggabilly.on('dragMove', (event, pointer, moveVector) => {
          ...
          // HACK
          this.emit('dragMove', { event, tabEl })
        })
      })
    }
